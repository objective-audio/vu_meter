...
void vu::ui_reference::_setup_flows(main_ptr_t &main) {
    this->_flows += this->_stepper.begin_minus_flow().receive_null(main->data.reference_decrement_receiver()).end();

    this->_flows += this->_stepper.begin_plus_flow().receive_null(main->data.reference_increment_receiver()).end();

    this->_flows += main->data.reference()
                        .chain()
                        .to([this](int32_t const &value) { return std::to_string(value) + " dB"; })
                        .receive(this->_stepper.text_receiver())
                        .sync();

    this->_flows += main->data.begin_is_reference_max_flow()
                        .to([](bool const &value) { return !value; })
                        .receive(this->_stepper.plus_enabled_receiver())
                        .sync();

    this->_flows += main->data.begin_is_reference_min_flow()
                        .to([](bool const &value) { return !value; })
